...
public class RobotContainer {
  ...
  private void configureButtonBindings() {
    ...
    // Top Taşıma
    new JoystickButton(m_driverController, 10).whileHeld(new FunnelCommand(m_funnel, -1));

    // Turret Reset
    new JoystickButton(m_driverController, 7).whenPressed(() -> m_turret.resetEncoder());
    
    // Turret Auto
    new JoystickButton(m_driverController, 3).whileHeld(new TurretMiddleCommand(m_turret, m_vision));
    ...
    // Turn Command
    new JoystickButton(m_operatorController, 1).whileHeld(new TurnCommand(m_robotDrive, m_gyro));
  }
